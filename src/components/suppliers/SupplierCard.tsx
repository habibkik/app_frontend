      ...
      <CardContent className="p-5" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-start gap-4 mb-4">
          <div className="h-14 w-14 rounded-lg bg-gradient-primary flex items-center justify-center flex-shrink-0">
            <span className="text-lg font-bold text-primary-foreground">{supplier.logo}</span>
          </div>
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-1">
              <h3 
                ...
                onClick={(e) => { e.stopPropagation(); onNameClick?.(supplier); }}
              >
                {supplier.name}
              </h3>
            ...
            <div className="flex items-center gap-3 text-sm text-muted-foreground">
              <span className="flex items-center gap-1"><MapPin className="h-3 w-3" />{supplier.location.city}, {supplier.location.country}</span>
              <span className="flex items-center gap-1"><Building2 className="h-3 w-3" />{supplier.industry}</span>
            </div>
            ...
            {supplier.isAIDiscovered && (
              <div className="flex items-center gap-2 mt-2">
                ...
                {supplier.matchScore && <Badge variant="outline" className="text-xs">{supplier.matchScore}% {t("supplierDetail.match")}</Badge>}
                {supplier.substituteOf && <Badge variant="secondary" className="text-xs">{t("supplierDetail.substitute")}</Badge>}
              </div>
            )}
          </div>
        </div>
        ...
        <div className="flex flex-wrap gap-1.5 mb-4">
          {supplier.specializations.slice(0, 3).map((spec) => (
            <Badge key={spec} variant="secondary" className="text-xs font-normal">{spec}</Badge>
          ))}
        </div>
        ...
        <div className="grid grid-cols-3 gap-2 py-3 border-y border-border mb-4">
          ...
          <div className="text-center">
            <span className="font-semibold text-sm">{fc(supplier.minOrderValue)}</span>
            <span className="text-xs text-muted-foreground block">{t("suppliers.details.minOrder")}</span>
          </div>
        </div>

        <div className="flex items-center gap-1.5 mb-4 overflow-hidden">
          {supplier.certifications.slice(0, 3).map((cert) => (
            <span key={cert} className="text-xs px-2 py-0.5 rounded bg-muted text-muted-foreground">{cert}</span>
          ))}
          {supplier.certifications.length > 3 && (
            <span className="text-xs text-muted-foreground">+{supplier.certifications.length - 3}</span>
          )}
        </div>

        <div className="flex gap-2">
          <Button className="flex-1" onClick={() => onContact(supplier)}>{t("suppliers.contactSupplier")}</Button>
          <Button variant="outline" onClick={() => onSave(supplier)}>{t("common.save")}</Button>
        </div>
      </CardContent>
    </Card>
  );
