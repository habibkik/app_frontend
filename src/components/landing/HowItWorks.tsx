...
const HowItWorks = () => {
  ...
  return (
    <section className="py-16 sm:py-28 bg-column relative overflow-hidden" id="how-it-works">
      <div className="container mx-auto px-4 sm:px-6 relative">
        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }} className="text-center max-w-3xl mx-auto mb-10 sm:mb-16">
          ...
          <p className="text-base sm:text-lg text-column-body">{t("howItWorks.subtitle")}</p>
        </motion.div>
          ...
          {steps.map((step, index) => (
            <motion.div key={step.step} initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: index * 0.12 }} className="relative">
              ...
              <div className="relative">
                <div className="absolute -top-2 -left-2 w-7 h-7 sm:w-8 sm:h-8 rounded-md border-2 border-column-navy flex items-center justify-center bg-column">
                  <span className="text-[10px] sm:text-xs font-bold text-column-navy">{step.step}</span>
                </div>
                <div className="w-16 h-16 sm:w-24 sm:h-24 rounded-2xl border border-gray-200 bg-gray-50 flex items-center justify-center mb-4 sm:mb-6">
                  <step.icon className="w-7 h-7 sm:w-10 sm:h-10 text-column-navy" />
                </div>
                <h3 className="text-base sm:text-xl font-semibold text-column-navy mb-2 sm:mb-3 tracking-tight">{step.title}</h3>
                <p className="text-xs sm:text-base text-column-body leading-relaxed">{step.description}</p>
              </div>
            </motion.div>
          ))}
        </div>
