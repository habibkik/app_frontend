...
function DashboardContent() {
  ...
  const handleAnalysisComplete = () => {
    switch (mode) {
      case "buyer": navigate("/dashboard/suppliers"); break;
      case "producer": navigate("/dashboard/bom"); break;
      case "seller": navigate("/dashboard/market"); break;
    }
  };
  ...
  return (
    <div className="space-y-8">
      <div className="flex items-start justify-between">
        <div>
          <motion.h1 key={mode} initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="text-2xl font-bold text-foreground">
            {modeLabel} {t("mainDashboard.title")}
          </motion.h1>
          <p className="text-muted-foreground mt-1">{t("mainDashboard.uploadText")}</p>
        </div>
      ...
      <Card className="border-2 border-dashed border-primary/20 bg-gradient-to-br from-primary/5 via-background to-background">
        <CardHeader className="text-center pb-2">
          <div className="flex items-center justify-center gap-2 mb-2">
            ...
            <CardTitle className="text-lg">{t("mainDashboard.aiAnalysis")}</CardTitle>
          </div>
        </CardHeader>
        <CardContent>
