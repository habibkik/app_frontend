...
export function ScenarioSimulator({ analysis, onScenarioChange }: ScenarioSimulatorProps) {
  ...
  // Find next volume discount threshold
        ...
        <CollapsibleContent>
          <CardContent className="space-y-6 pt-0">
            ...
            <div className="space-y-3">
              ...
              <Slider
                value={[params.productionVolume]}
                onValueChange={([v]) => updateParam("productionVolume", v)}
                min={100}
                max={10000}
                step={100}
                className="w-full"
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                ...
                <span className={`text-sm font-medium ${
                  params.costIncreasePercent > 0 ? "text-red-500" : "text-foreground"
                }`}>
                  {params.costIncreasePercent > 0 ? "+" : ""}{params.costIncreasePercent}%
                </span>
              </div>
              <Slider
                value={[params.costIncreasePercent]}
                onValueChange={([v]) => updateParam("costIncreasePercent", v)}
                min={0}
                max={30}
                step={1}
                className="w-full"
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                ...
                <span className="text-sm font-medium text-primary">
                  ${params.laborCostPerHour}/hr
                </span>
              </div>
              <Slider
                value={[params.laborCostPerHour]}
                onValueChange={([v]) => updateParam("laborCostPerHour", v)}
                min={15}
                max={50}
                step={1}
                className="w-full"
                ...
                >
                  <div className="flex items-center justify-between mb-3">
                    <div>
                      ...
                      <p className="text-2xl font-bold text-foreground">
                        ${result.projectedUnitCost.toFixed(2)}
                      </p>
                    </div>
                  ...
                  {nextDiscount && !result.volumeDiscountApplied && (
                    <p className="text-xs text-muted-foreground mt-2">
                      💡 Volume discount of {nextDiscount.discount}% kicks in at{" "}
                      {nextDiscount.threshold.toLocaleString()} units
                    </p>
                  )}

                  {nextDiscount && result.volumeDiscountApplied && (
                    <p className="text-xs text-muted-foreground mt-2">
                      💡 Next discount tier ({nextDiscount.discount}%) at{" "}
                      {nextDiscount.threshold.toLocaleString()} units
                    </p>
                  )}
                </motion.div>
