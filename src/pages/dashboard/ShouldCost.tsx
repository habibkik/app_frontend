...
import {
  ...
  RotateCcw, Download, Copy, CheckCircle2,
} from "lucide-react";
...
export default function ShouldCostPage() {
  ...
  const costs = useMemo(() => {
    ...
    const tooling = inputs.toolingLifeUnits > 0
      ? inputs.toolingCost / inputs.toolingLifeUnits
      : 0;
        ...
        <motion.div variants={item} className="flex items-center justify-between flex-wrap gap-3">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Estimate expected production cost to benchmark supplier quotes
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <motion.div variants={item} className="lg:col-span-2 space-y-4">
              ...
              <CardContent className="space-y-3">
                <div className="flex items-center gap-3">
                  ...
                  <Input
                    ...
                    onChange={(e) => set("productName", e.target.value as any)}
            ...
            <Card>
              <CardHeader className="pb-3">
                <CardTitle className="text-base flex items-center gap-2">
                  <div className="h-2.5 w-2.5 rounded-full" style={{ background: PIE_COLORS[0] }} />
                  Material
                </CardTitle>
            ...
            <Card>
              <CardHeader className="pb-3">
                <CardTitle className="text-base flex items-center gap-2">
                  <div className="h-2.5 w-2.5 rounded-full" style={{ background: PIE_COLORS[1] }} />
                  Labor
                </CardTitle>
              </CardHeader>
              <CardContent className="space-y-3">
            ...
            <Card>
              <CardHeader className="pb-3">
                <CardTitle className="text-base flex items-center gap-2">
                  <div className="h-2.5 w-2.5 rounded-full" style={{ background: PIE_COLORS[2] }} />
                  Machine
                </CardTitle>
              </CardHeader>
              <CardContent className="space-y-3">
            ...
            <Card>
              <CardHeader className="pb-3">
                <CardTitle className="text-base flex items-center gap-2">
                  <div className="h-2.5 w-2.5 rounded-full" style={{ background: PIE_COLORS[3] }} />
                  Tooling
                </CardTitle>
              </CardHeader>
              <CardContent className="space-y-3">
            ...
            {/* Overhead, scrap, packaging, logistics */}
            <Card>
              <CardHeader className="pb-3">
                <CardTitle className="text-base">Overhead, Scrap & Extras</CardTitle>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    <Label className="text-sm">Overhead</Label>
                    <Badge variant="secondary">{inputs.overheadPercent}%</Badge>
                  </div>
                  <Slider value={[inputs.overheadPercent]} onValueChange={([v]) => set("overheadPercent", v)} min={0} max={50} step={1} />
                </div>
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    <Label className="text-sm">Scrap Rate</Label>
                    <Badge variant="secondary">{inputs.scrapPercent}%</Badge>
                  </div>
            ...
            <Card>
              <CardHeader className="pb-3">
                <CardTitle className="text-base">Supplier Margin</CardTitle>
              </CardHeader>
              <CardContent>
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    <Label className="text-sm">Target Margin</Label>
                    <Badge variant="secondary">{inputs.marginPercent}%</Badge>
                  </div>
          ...
          <motion.div variants={item} className="space-y-4">
              ...
              <CardContent className="space-y-4">
                <div className="text-center py-3">
                  ...
                  <p className="text-xs text-muted-foreground mt-1">
                    Production: {fc(costs.totalCost)} + Margin: {fc(costs.margin)}
                  </p>
                </div>
                ...
                <div className="space-y-2 text-sm">
                  ...
                  <div className="flex justify-between font-medium">
                    ...
                    <span>{fc(costs.totalCost)}</span>
                  </div>
                  <div className="flex justify-between text-muted-foreground">
                    <span>Margin ({inputs.marginPercent}%)</span>
                    <span>{fc(costs.margin)}</span>
                  </div>
                  <div className="flex justify-between font-bold text-primary">
                    <span>Should-Cost</span>
                    <span>{fc(costs.shouldCost)}</span>
                  </div>
                </div>
              ...
              <CardContent className="h-[240px]">
                {pieData.length > 0 ? (
                  ...
                  <div className="h-full flex items-center justify-center text-sm text-muted-foreground">
                    Enter cost inputs to see distribution
                  </div>
                )}
              </CardContent>
            </Card>
            ...
            <Card>
              <CardHeader className="pb-2">
                <CardTitle className="text-base">Cost Waterfall</CardTitle>
              </CardHeader>
              <CardContent className="h-[220px]">
                {breakdownData.length > 0 ? (
                  ...
                  <div className="h-full flex items-center justify-center text-sm text-muted-foreground">
                    Enter cost inputs to see waterfall
                  </div>
                )}
              </CardContent>
            </Card>
